...
void Text::generateTexture()
{
	...
	{
		// Free last loaded image if regenerate texture to prevent memory leak	
...
void Text::draw(RenderWindow& win, int n)
{
	if (n == ALLREGION)
	{
		SDL_RenderSetViewport(win.getRenderer(), NULL);
	}
	else
	{
		SDL_RenderSetViewport(win.getRenderer(), win.getRegion(n));
	}
